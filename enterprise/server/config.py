...
class SaaSServerConfig(ServerConfig):
    ...
    monitoring_listener_class: str = (
        os.environ.get(
            "MONITORING_LISTENER_CLASS",
            "server.saas_monitoring_listener.SaaSMonitoringListener"
            if os.environ.get("ENABLE_TELEMETRY_INFRASTRUCTURE", "false").lower()
            == "true"
            else None,
        )
        or ""
    ...
    def verify_config(self):
        ...
        # PostHog requirement made optional - controlled by ENABLE_POSTHOG_TELEMETRY
        enable_posthog_telemetry = (
            os.environ.get("ENABLE_POSTHOG_TELEMETRY", "false").lower() == "true"
        )
        if enable_posthog_telemetry and not self.posthog_client_key:
            raise ValueError(
                "PostHog telemetry enabled but POSTHOG_CLIENT_KEY missing in env"
            )
